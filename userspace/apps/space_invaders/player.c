...
// Moves the tank on screen by taking its current position and either incrementing or decrementing
// the current x position based on buttons input
// @param - buttons is a uint32_t that when masked with the right #define statement will return t/f
...
// Fires a bullet from the tanks current position. Offsets the starting point a little to maintain
// the image of firing from the turret
void fire_bullet() {
    bullet_player_fire(player_tank.pos_x + TANK_HEIGHT * SCALE, player_tank.pos_y - GROUND_OFFSET);
}

// Polls all the alien bullets to see if any are currently within the range and position of the tank
// in this tick RETURN: True if a bullet struck the tank, false otherwise
bool check_bullet() {
    ...
    for (uint8_t i = 0; i < ALIEN_BULLETS; i++) {
        ...
        // If the bottom of the selected alien bullet is within the correct height and x span of the
        // tank
        if ((y_pos + GROUND_OFFSET >= player_tank.pos_y) &&
            (x_pos + ALIEN_BULLET_WIDTH >= player_tank.pos_x && x_pos - ALIEN_BULLET_WIDTH <= (player_tank.pos_x + TANK_WIDTH * SCALE))) {
            // Report a hit
...
void explosion() {
    ...
    if (player_tank.is_alive) {
        ...
        hdmi_draw_sprite(sprite_tank_17x8, TANK_WIDTH, TANK_HEIGHT, SCALE, back, back,
                         player_tank.pos_y, player_tank.pos_x);
    ...
    if (explosion1) {
        ...
        hdmi_draw_sprite(sprite_tank_explosion2_15x8, TANK_NORM_WIDTH, TANK_HEIGHT, SCALE, back,
                         back, player_tank.pos_y, player_tank.pos_x);
        hdmi_draw_sprite(sprite_tank_explosion1_15x8, TANK_NORM_WIDTH, TANK_HEIGHT, SCALE, tank,
                         back, player_tank.pos_y, player_tank.pos_x);
    }
    // If second sprite is in use
    else {
        // Erase first sprite and draw second
        hdmi_draw_sprite(sprite_tank_explosion1_15x8, TANK_NORM_WIDTH, TANK_HEIGHT, SCALE, back,
                         back, player_tank.pos_y, player_tank.pos_x);
        hdmi_draw_sprite(sprite_tank_explosion2_15x8, TANK_NORM_WIDTH, TANK_HEIGHT, SCALE, tank,
                         back, player_tank.pos_y, player_tank.pos_x);
    }
...
void player_tick() {
    ...
    case wait_for_input_st: // Waiting for user input
        ...
        if ((buttons & BUTTONS_3_MASK) || (buttons & BUTTONS_2_MASK)) {
    ...
    case hit_received_st: // Have been struck by a bullet
        // If the duration of explosion animation has maxed out, reset counters, tank position, and
        // flags, go to init
        if (explosion_counter == EXPLOSION_CNT_MAX) {
            explosion_counter = CNT_RESET;
            hdmi_draw_sprite(sprite_tank_gone_15x8, TANK_NORM_WIDTH, TANK_HEIGHT, SCALE, back, back,
                             player_tank.pos_y, player_tank.pos_x);
            player_tank.is_alive = true;
            explosion1 = true;
            currentPlayerHandler = init_st;
