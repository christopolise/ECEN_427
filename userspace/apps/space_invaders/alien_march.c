    ...
    for (uint16_t col = 0; col < ALIENS_NUM_COLS; ++col) {
      ...
      if (aliens.aliens[col][row].is_alive) {
        ...
        if (out) {
          ...
          if (row == ROW0) {
            hdmi_draw_sprite(sprite_alien_top_out_12x8, ALIEN_SPRITE_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x);
          } else if (row < ROW3) {
            hdmi_draw_sprite(sprite_alien_middle_out_12x8, ALIEN_ERASER_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, back, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x - 4);
            hdmi_draw_sprite(sprite_alien_middle_out_12x8, ALIEN_ERASER_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, back, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x + 23);
            hdmi_draw_sprite(sprite_alien_middle_out_12x8, ALIEN_SPRITE_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x);
          } else {
            hdmi_draw_sprite(sprite_alien_bottom_out_12x8, ALIEN_ERASER_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, back, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x - 4);
            hdmi_draw_sprite(sprite_alien_bottom_out_12x8, ALIEN_ERASER_LEN,
                             ...
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x + 23);
            hdmi_draw_sprite(sprite_alien_bottom_out_12x8, ALIEN_SPRITE_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x);
          }
          ...
          // different rows have different sprites
          if (row == ROW0) {
            hdmi_draw_sprite(sprite_alien_top_in_12x8, ALIEN_SPRITE_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x);
          } else if (row < ROW3) {
            ...
            hdmi_draw_sprite(sprite_alien_middle_out_12x8, ALIEN_ERASER_LEN,
                             ...
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x + 23);
            hdmi_draw_sprite(sprite_alien_middle_in_12x8, ALIEN_SPRITE_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x);
          } else {
            ...
            hdmi_draw_sprite(sprite_alien_bottom_out_12x8, ALIEN_ERASER_LEN,
                             ...
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x + 23);
            hdmi_draw_sprite(sprite_alien_bottom_in_12x8, ALIEN_SPRITE_LEN,
                             ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                             aliens.aliens[col][row].pos_y,
                             aliens.aliens[col][row].pos_x);
          }
  ...
  for (uint16_t row = 0; row < ALIENS_NUM_ROWS; ++row) {
    ...
    for (uint16_t col = 0; col < ALIENS_NUM_COLS; ++col) {
      ...
      if (aliens.aliens[col][row].is_alive) {
        ...
        uint16_t alien_x_max =
            aliens.aliens[col][row].pos_x + ALIEN_SPRITE_LEN * SCALE;
        uint16_t alien_y_max =
            aliens.aliens[col][row].pos_y + ALIEN_SPRITE_HEIGHT * SCALE;
        ...
        if (bullet_x >= alien_x_min && bullet_x <= alien_x_max &&
          ...
          hdmi_draw_sprite(sprite_alien_bottom_in_12x8, ALIEN_SPRITE_LEN,
                           ...
                           aliens.aliens[col][row].pos_y,
                           aliens.aliens[col][row].pos_x);
          hdmi_draw_sprite(sprite_alien_explosion_12x10, ALIEN_SPRITE_LEN,
                           ALIEN_SPRITE_HEIGHT, SCALE, fore, back,
                           aliens.aliens[col][row].pos_y,
                           aliens.aliens[col][row].pos_x);
  ...
  if (cnt == ALIEN_MARCH_SPEED) {
    ...
    for (uint16_t row = 0; row < ALIENS_NUM_ROWS; ++row) {
      ...
      for (uint16_t col = 0; col < ALIENS_NUM_COLS; ++col) {
        ...
        if ((aliens.aliens[col][row].pos_x >=
             HDMI_DISPLAY_WIDTH - SCREEN_BUF_LEFT) ||
    ...
    for (uint16_t row = 0; row < ALIENS_NUM_ROWS; ++row) {
      ...
      for (uint16_t col = 0; col < ALIENS_NUM_COLS; ++col) {
        ...
        if (y_change) {
          hdmi_draw_sprite(sprite_alien_top_out_12x8, ALIEN_SPRITE_LEN, 2,
                           SCALE, back, back, aliens.aliens[col][row].pos_y,
                           aliens.aliens[col][row].pos_x);

          // if the alien is alive and has reached the bunkers
          if (aliens.aliens[col][row].is_alive &&
              aliens.aliens[col][row].pos_y > BUNKER_Y_POS) {
    ...
    if (alien_to_erase_x != ERROR && alien_to_erase_y != ERROR) {
      hdmi_draw_sprite(sprite_alien_explosion_12x10, EXPLOSION_ERASER_LEN,
                       EXPLOSION_HEIGHT, SCALE, back, back,
                       aliens.aliens[alien_to_erase_x][alien_to_erase_y].pos_y,
                       aliens.aliens[alien_to_erase_x][alien_to_erase_y].pos_x -
                           ERASER_OFFSET);
    }
  ...
  if (cnt == BULLET_WAIT) {
    ...
    for (uint8_t bullet = 0; bullet < ALIEN_BULLETS; ++bullet) {
      ...
      if (!bullet_is_alien_bullet_active(bullet)) {
        ...
        bullet_alien_fire(bullet, bullet_x + BULLET_OFFSET,
