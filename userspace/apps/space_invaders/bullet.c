...
// @param - num_of_alien is a uint8_t that denotes which alien bullet we are
// querying RETURN: true if the alien bullet is active false otherwise
...
// with something
// @param - num_of_alien is uint8_t that describes which of the 4 active alien
// bullets is being referenced
void bullet_set_alien_bullet_collide(uint8_t num_of_alien) {
  hdmi_draw_sprite(sprite_alienbullet2_gone_3x5, ALIEN_BULLET_WIDTH,
                   BULLET_LENGTH, SCALE_FACTOR, backgr, backgr,
                   aliens_bullets[num_of_alien].pos_y,
...
void bullet_set_player_bullet_collide() {
  hdmi_draw_sprite(sprite_tankbullet_1x5, PLAYER_BULLET_WIDTH, BULLET_LENGTH,
                   SCALE_FACTOR, backgr, backgr, play_bullet.pos_y,
                   play_bullet.pos_x);
...
// @param - num_of_alien is uint8_t that describes which of the 4 active alien
// bullets is being referenced
void bullet_get_alien_bullet_coordinates(uint16_t *x_pos, uint16_t *y_pos,
                                         uint8_t num_of_alien) {
  *x_pos = aliens_bullets[num_of_alien].pos_x;
  *y_pos = aliens_bullets[num_of_alien].pos_y;
}

// Allows caller to set the coordinates of the starting point of an alien shot.
// The purpose is to allow caller to do their own randomization of shots and use
// this function as the interface to actually locate and shoot bullet from
// coordinates
...
// @param - num_of_alien is uint8_t that describes which of the 4 active alien
// bullets is being referenced
void bullet_set_alien_bullet_coordinates(uint16_t x_pos, uint16_t y_pos,
                                         uint8_t num_of_alien) {
...
// Determines which alien bullet out of the 4 choices available will fire the
// bullet and where
// @param - alien_num is uint8_t that describes which of the 4 active alien
// bullets is being referenced
// @param - x_pos is uint16_t that describes x-coordinate of new bullet origin
// @param - y_pos is uint16_t that describes y-coordinate of new bullet origin
void bullet_alien_fire(uint8_t alien_num, uint16_t x_pos, uint16_t y_pos) {
  if (!aliens_bullets[alien_num].is_alive) {
    // This block will only run if the current alien bullet selected isn't
    // already active
...
void bullet_init() {
  ...
  for (uint8_t i = 0; i < ALIEN_BULLETS; i++) {
    // For all existing alien bullets, we will set their availablity and
    // orientation to default to false
...
// This function determines the rate and the trajectory a fired bullet from the
// tank has
void player_shot_fly() {
  ...
  if ((play_bullet.pos_y > BANNER_BOUNDARY) && play_bullet.is_alive) {
    ...
    // Draw bullet
    hdmi_draw_sprite(sprite_tankbullet_1x5, PLAYER_BULLET_WIDTH, BULLET_LENGTH,
                     SCALE_FACTOR, fore, backgr, play_bullet.pos_y,
                     play_bullet.pos_x);
  }
...
// Helper function that moves along a bullet along a certain path and with a
// particular sprite or set of them, this is what should allow alien bullets to
// change shape
// @param - num_of_alien is uint8_t that describes which of the 4 active alien
// bullets is being referenced
// @param - sprite_name is an array of uint32_t which, when passed through a
// print sprite function, will print the bitmap onto the screen
void aliens_animation_helper(uint8_t num_of_alien,
                             const uint32_t *sprite_name) {
  // If the bullet is still on the screen and is still registered as not having
  // hit anything
  if ((aliens_bullets[num_of_alien].pos_y + GROUND_OFFSET <
       (HDMI_DISPLAY_HEIGHT - GRASS_LIM)) &&
      aliens_bullets[num_of_alien].is_alive) {
    // erase current bullet at position
    hdmi_draw_sprite(sprite_name, ALIEN_BULLET_WIDTH, BULLET_LENGTH,
                     SCALE_FACTOR, backgr, backgr,
                     aliens_bullets[num_of_alien].pos_y,
                     aliens_bullets[num_of_alien].pos_x);
    // Increment bullet's position
...
// Function that causes an alien bullet to fly and alternate the pattern as it
// flies and the type of bullet based on the eveness of the number
